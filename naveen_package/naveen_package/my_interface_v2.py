...
class CompetitionInterface(Node):
    ...
    def __init__(self, enable_moveit=True):
        ...
        self.new_cb_group = MutuallyExclusiveCallbackGroup()
    ...
    def _orders_cb(self, msg: OrderMsg):
        ...
        # checking for high priority orders
        if order.order_priority == True:
            self._flag = True
    ...
    def _breakbeam_cb(self, msg: BreakBeamStatusMsg):
        ...
        detection_time = self.get_clock().now().nanoseconds
        prev_distance = 0
        count = 0
        
        if break_beam_status:
            with self._lock:
                for part in self._conveyor_part_detected:
                    part_pose = Pose()
                    part_pose = multiply_pose(self._conveyor_camera_pose, part.pose)
                    # For determining the offset of the part in the conveyor belt
                    distance = abs(part_pose.position.y - self._breakbeam_pose.position.y)
                    if count == 0:
                        part_to_add = part
                        detection_time = self.get_clock().now().nanoseconds
                        ...
                        if distance < prev_distance:
                            ...
                            detection_time = self.get_clock().now().nanoseconds
                            prev_distance = distance
                self._conveyor_parts[detection_time] = part_to_add
    ...
    def _conveyor_camera_cb(self, msg: AdvancedLogicalCameraImage):
        ...
        self._conveyor_part_detected = msg.part_poses
    ...
    def floor_robot_pick_conveyor_part(self, part_to_pick: PartMsg):
        ...
        for parts in self._conveyor_parts_expected:
            if parts.part.type == part_to_pick.type and parts.part.color == part_to_pick.color:
                self.get_logger().info(f"Attempting to pickup a {self._part_colors[parts.part.color]} part of type {self._part_types[parts.part.type]} from the conveyor")
                break
            else:
        ...
        part_pose = Pose()
        # New implementation
        elapsed_time = rclpy.time.Duration(seconds=0)
        time_to_pick = Duration()
        detection_time = rclpy.time.Time()
        ...
        while not part_picked and num_tries < 3:
            self.floor_robot_move_to_joint_position("floor_conveyor_js_")
            # New code implementation
            while not found_part:
                with self._lock:
                    conveyor_parts = copy(self._conveyor_parts)
                    for key, value in conveyor_parts.items():
                        part = value
                        if part.part.type == part_to_pick.type and part.part.color == part_to_pick.color:
                            part_pose = multiply_pose(self._conveyor_camera_pose, part.pose)
                            detection_time = key
                            # detection_time_ns = detection_time * 1e9
                            elapsed_time = self.get_clock().now().nanoseconds - detection_time
                            elapsed_time_to_sec = elapsed_time/1e9
                            current_part_position = part_pose.position.y - (elapsed_time_to_sec * self._conveyor_speed)
                            if current_part_position > 0:
                                time_to_pick = current_part_position/self._conveyor_speed
                                time_to_pick_ns = time_to_pick * 1e9
                                if time_to_pick > 5.0:
                                    found_part = True
                                    del self._conveyor_parts[key]
                                    break
                            del self._conveyor_parts[key]
                    self.get_logger().info(f"Waiting for {self._part_colors[part_to_pick.color]} {self._part_types[part_to_pick.type]} to arrive on the conveyor")
            part_rotation = rpy_from_quaternion(part_pose.orientation)[2]
            gripper_orientation = quaternion_from_euler(0.0,pi,part_rotation)
            with self._planning_scene_monitor.read_write() as scene:
                robot_pose = Pose()
            ...
            current_part_position = part_pose.position.y - (elapsed_time_to_sec*self._conveyor_speed)
            if current_part_position < 0:
                self.get_logger().info("Part has passed the pick up location")
                found_part = False
                num_tries +=1
                continue
            # For finding trajectory
            waypoints = [build_pose(part_pose.position.x, robot_pose.position.y,
                    ...
                    gripper_orientation)]
            validity, trajectory = self._call_get_cartesian_path_conveyor(waypoints, 0.5, 0.5, False)     
            if not validity:
                found_part = False
                num_tries +=1
                continue
            trajectory_time = trajectory.joint_trajectory.points[-1].time_from_start.nanosec
            # trajectory_time_ns = trajectory_time * 1e9
            while (self.get_clock().now().nanoseconds < (detection_time + elapsed_time + time_to_pick_ns - trajectory_time) - 3.75e8):
            ...
            # start_time_to_sec = start_time/1e9
            thres_time = 2.0*1e9
            while not self._floor_robot_gripper_state.attached:
                if self.get_clock().now().nanoseconds - start_time > thres_time:
                    break
